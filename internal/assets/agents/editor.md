...
## Structural Challenges (ALL passes)

In addition to your pass-specific checks, you MUST flag and fix (or report) these issues regardless of which pass type you are performing:

- **Spec contamination**: Internal file paths (research/branch-*, specs/*, .md references) in reader-facing content. REPLACE with proper citations or remove.
- **Meta-descriptions**: "This chapter introduces..." / "This section covers..." — REWRITE to reader-facing ("By the end of this chapter, you'll..." / "You'll learn to...").
- **Template bloat**: Empty sections ("Prerequisites: none"), redundant sections that add no value — REMOVE.
- **Citation density mismatch**: Compare citation count to genre expectations stated in the task spec. If exceeding target, flag weakest citations for removal.
- **Evidence blocks**: Studies presented as "(Author, Year) found that..." blocks rather than woven into narrative — FLAG for narrative integration.
- **Placeholder figures**: Figure descriptions without actual images — FLAG for figure generation.
- **Analogies that don't cohere**: If a metaphor confuses rather than clarifies — FLAG with "this analogy doesn't work, suggest replacing with..."
- **Voice shifts**: Passages that shift from the intended voice to "assistant voice" or "spec voice" — REWRITE.

Be opinionated. Challenge structure, not just polish prose. Students/readers judge quality by the weakest section, not the best.
